---
title: "STged: Gene expression Deconvolution for human PDAC SRT datasets"
output:
  html_document: default
  pdf_document: default
  fontsize: 10pt
author: Jia-Juan Tu
---


```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

##Set the size of the figures
```{r, fig.height=7, fig.width=6, size="small", fig.align="center"}

```

## Set environment of Python modual
```{r}
#Please check the environment on you PC
python_env <- 'C:/Users/visitor01/.conda/envs/stged/python.exe'

reticulate::use_python(python = python_env, required = TRUE)

```

## Load function
```{r}
source("./code/utils.R")

```


# load data sets
```{r}
inputx <- readRDS("./data/PDAC_STged.Rds")

```

# Run STged step by step
**Note**: There are three steps in STged. We here give a small example to show how to run the code step by step.

## Step 1: clear data
```{r}

datax = data_process(sc_exp = inputx$sc_exp, sc_label = inputx$sc_label, 
                     spot_exp = inputx$spot_exp, spot_loc = inputx$spot_loc,
                     depthscale = 1, gene_det_in_min_cells_per = 0.01, 
                     expression_threshold = 0,
                     nUMI =  100, verbose = FALSE, clean.only = FALSE)

## Winsorize expression values to prevent outliers  
truncate = TRUE
if(truncate){
  qt = 0.01
  datax$sc_exp  =  winsorize(x =  datax$sc_exp, qt = qt)
  datax$spot_exp  =  winsorize(x =  datax$spot_exp, qt = qt)
}

```

## Step 2: construct spatial correlation structures
```{r}

cat("Construct spatial correlation", "\n")
L.mat = dis_weight(spot_loc = datax$spot_loc, spot_exp = datax$spot_exp, k = 6, 
                   quantile_prob_bandwidth = 1/3, method = "Hex", 
                   coord_type = "grid")


```

## Step 3: construct reference gene matrix
```{r}
cat("Construct reference gene matrix", "\n")
ref_exp = create_group_exp(sc_exp = datax$sc_exp, sc_label = datax$sc_label)

#the corresponding cell type proportion
beta =  inputx$beta[colnames(datax$spot_exp),]

```


## Step 4: run the main model
```{r}
cat("Run the STged", "\n")

start_time <- Sys.time()
model.est = tuning.sel.ADMM(srt_exp = datax$spot_exp, ref_exp = ref_exp, 
                            beta.type = beta, L = L.mat$dis_weight, lambda1 =  NULL, lambda2 =  NULL, 
                            cutoff = 0.05, epsilon = 1e-3, 
                            maxiter = 20, rho = 1,  rho.incr = 1.05, rho.max = 1e10)
end_time <- Sys.time()

cat("Run time of STged", end_time - start_time,"\n")

```


## Alternatively, we can run the STged in one step
```{r eval=FALSE, include=TRUE}

start_time <- Sys.time()

model.est = STged(sc_exp = inputx$sc_exp, sc_label = inputx$sc_label, 
                  spot_exp = inputx$spot_exp, spot_loc = inputx$spot_loc,  beta = inputx$beta,
                  gene_det_in_min_cells_per = 0.01, expression_threshold = 0,
                  nUMI = 100, verbose = FALSE, clean.only = FALSE, python_env = python_env,
                  truncate = TRUE, qt = 0.01,
                  knei = 6,  methodL =  "Hex",coord_type = "grid", quantile_prob_bandwidth = 1/3,
                  lambda1 = NULL, lambda2 = NULL, cutoff = 0.05,  rho = 1,  rho.incr = 1.05,
                  rho.max = 1e10, maxiter = 20,  epsilon = 1e-5)

end_time <- Sys.time()
print(end_time - start_time)

print(1)

```


